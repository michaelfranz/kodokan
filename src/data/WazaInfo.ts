...
const wazaClassificationMap: Map<string, string[]> = new Map([
    ...
    [
        'Te-Waza',
        ['Ippon-Seoi-Nage', 'Kata-Guruma', 'Sukui-Nage', 'Sumi-Gaeshi', 'Sumi-Otoshi', 'Tai-Otoshi', 'Uki-Otoshi'],
    ],
    [
        'Yoko-Sutemi-Waza',
        [
...
export default class TechniqueInfo {
    ...
    private constructor() {
        ...
        const classifications = wazaClassificationList.filter(name => name !== 'All')

        classifications.forEach(classification => {
            const techniques = wazaClassificationMap.get(classification)
            if (techniques === undefined) {
                throw new Error('Problem with data: techniques undefined for classification===' + classification)
            }
            techniques.forEach(technique => this.wazaTerms.add(technique))
        })
        const allTechniques = Array.from(this.wazaTerms).sort((a, b) => 0 - (a < b ? 1 : -1))
        wazaClassificationMap.set('All', allTechniques)

        // Initialise all kyo techniques ("gokyo")
        kyoWazaTermsMap.forEach(kyoWaza => kyoWaza.forEach(name => this.kyoWazaTerms.add(name)))
    }

    public isWazaTerm = (term: string): boolean => this.wazaTerms.has(term)

    public isClassification = (term: string): boolean => wazaClassificationList.includes(term)

    public classificationForWazaTerm = (wazaTerm: string): string => {
        const allClassifications = wazaClassificationList.filter(classification => classification !== 'All')
        return allClassifications.find(classification => wazaClassificationMap.get(classification).includes(wazaTerm))
    }
